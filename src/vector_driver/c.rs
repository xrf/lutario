pub mod ffi {
    ...
    pub type VectorOperation = unsafe extern fn(*mut raw::c_void,
                                                *mut VectorAccum,
                                                *const VectorAccum,
                                                libc::size_t,
    ...
    pub struct VectorDriverVt {
        pub len: unsafe extern fn(*const VectorDriverBase) -> libc::size_t,
        pub try_new: unsafe extern fn(*const VectorDriverBase) -> *mut Vector,
        pub del: unsafe extern fn(*const VectorDriverBase, *mut Vector),
        pub operate: unsafe extern fn(*const VectorDriverBase,
                                      *mut VectorAccum,
                                      VectorAccumType,
                                      VectorOperation,
                                      *mut raw::c_void,
...
impl<D: VectorDriver<Item=f64> + Sized> CVectorDriver<D> {
    ...
    unsafe extern fn len(d: *const ffi::VectorDriverBase) -> libc::size_t {
        abort_on_unwind(|| {
            let d = &*(d as *const D);
            d.len()
        })
    ...
    unsafe extern fn try_new(d: *const ffi::VectorDriverBase)
                             -> *mut ffi::Vector {
        abort_on_unwind(|| {
            let d = &*(d as *const D);
    ...
    unsafe extern fn del(_: *const ffi::VectorDriverBase, v: *mut ffi::Vector) {
        abort_on_unwind(|| {
            if !v.is_null() {
    ...
    unsafe extern fn operate(d: *const ffi::VectorDriverBase,
        ...
        abort_on_unwind(|| {
            ...
            let mut mut_vectors: Vec<_> =
                slice::from_raw_parts_mut(vectors, num_vectors)
                .into_iter().map(|p| &mut *(*p as *mut D::Vector)).collect();
            if accum_type >= 0 {
                let accum_len =
                    accum_type.value_into().expect("accum_len overflows usize");
                let accum = slice::from_raw_parts_mut(
                    accum as *mut u8,
                    accum_len,
                );
                let mut accum_vec = accum.to_owned();
                d.operate_on(&mut accum_vec, offset,
                             &[], &mut mut_vectors,
                             |accum, val, offset, slices, mut_slices|
                             wrapper(f, f_ctx, accum_len, num_vectors,
                                     accum, val, offset, slices, mut_slices));
                accum.copy_from_slice(&accum_vec)
            } else {
                let accum_len =
                    accum_type.checked_neg()
                    .expect("negation of accum_type overflows c_int")
                    .value_into().expect("accum_len overflows usize");
                let accum = slice::from_raw_parts_mut(
                    accum as *mut f64,
                    accum_len,
                );
...
fn wrapper<S>(f: ffi::VectorOperation,
              f_ctx: UnsafeSync<*mut raw::c_void>,
              accum_len: usize, num_vectors: usize,
              accum: &mut Vec<S>, val: Vec<S>, offset: usize,
              slices: &[&[f64]], mut_slices: &mut [&mut [f64]]) {
    abort_on_unwind(|| {

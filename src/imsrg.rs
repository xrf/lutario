...
use super::half::Half;
use super::j_scheme::{
    clone_mop_j012_from_tri_slice, clone_mop_j012_to_tri_slice, extent_mop_j012_as_tri,
    new_mop_j012, op200_to_op211, op211_to_op200, JScheme, MopJ012, OpJ100, OpJ200, StateJ10,
};
...
/// ```
pub fn c011(alpha: f64, a1: &OpJ100<f64>, b1: &OpJ100<f64>, c0: &mut f64) {
...
/// ```
pub fn c022(alpha: f64, a2: &OpJ200<f64>, b2: &OpJ200<f64>, c0: &mut f64) {
...
/// ```
pub fn c111(alpha: f64, a1: &OpJ100<f64>, b1: &OpJ100<f64>, c1: &mut OpJ100<f64>) {
    let scheme = a1.scheme();
...
/// ```
pub fn c112(alpha: f64, a1: &OpJ100<f64>, b2: &OpJ200<f64>, c1: &mut OpJ100<f64>) {
    let scheme = a1.scheme();
    for p in scheme.states_10(&occ::ALL1) {
        for q in p.costates_10(&occ::ALL1) {
            for i in scheme.states_10(&[occ::I]) {
                for a in i.costates_10(&[occ::A]) {
                    for jap in Half::tri_range_2((a.j(), p.j()), (i.j(), q.j())) {
                        for ap in a.combine_with_10(p, jap) {
                            for iq in i.combine_with_10(q, jap) {
                                c1.add(
                                    p,
                                    q,
                                    alpha * ap.jweight(2) / p.jweight(2)
                                        * a1.at(i, a)
                                        * b2.at(ap, iq),
                                );
                            }
...
/// ```
pub fn c121(alpha: f64, a2: &OpJ200<f64>, b1: &OpJ100<f64>, c1: &mut OpJ100<f64>) {
    let scheme = a2.scheme();
    for p in scheme.states_10(&occ::ALL1) {
        for q in p.costates_10(&occ::ALL1) {
            for a in scheme.states_10(&[occ::A]) {
                for i in a.costates_10(&[occ::I]) {
                    for jip in Half::tri_range_2((i.j(), p.j()), (a.j(), q.j())) {
                        for ip in i.combine_with_10(p, jip) {
                            for aq in a.combine_with_10(q, jip) {
                                c1.add(
                                    p,
                                    q,
                                    alpha * ip.jweight(2) / p.jweight(2)
                                        * a2.at(ip, aq)
                                        * b1.at(a, i),
                                );
                            }
...
/// ```
pub fn c1220(alpha: f64, a2: &OpJ200<f64>, b2: &OpJ200<f64>, c1: &mut OpJ100<f64>) {
    let scheme = a2.scheme();
    for p in scheme.states_10(&occ::ALL1) {
        for q in p.costates_10(&occ::ALL1) {
            for a in scheme.states_10(&[occ::A]) {
                        ...
                        for aq in a.combine_with_10(q, jij) {
                            for ij in ap.costates_20(&[occ::II]) {
                                c1.add(
                                    p,
                                    q,
                                    -alpha / 2.0 * ij.jweight(2) / p.jweight(2)
                                        * a2.at(ij, aq)
                                        * b2.at(ap, ij),
                                );
                            }
...
/// ```
pub fn c1221(alpha: f64, a2: &OpJ200<f64>, b2: &OpJ200<f64>, c1: &mut OpJ100<f64>) {
    let scheme = a2.scheme();
    for p in scheme.states_10(&occ::ALL1) {
        for q in p.costates_10(&occ::ALL1) {
            for i in scheme.states_10(&[occ::I]) {
                        ...
                        for iq in i.combine_with_10(q, jip) {
                            for ab in ip.costates_20(&[occ::AA]) {
                                c1.add(
                                    p,
                                    q,
                                    alpha / 2.0 * ip.jweight(2) / p.jweight(2)
                                        * a2.at(ip, ab)
                                        * b2.at(ab, iq),
                                );
                            }
...
/// ```
pub fn c212(alpha: f64, a1: &OpJ100<f64>, b2: &OpJ200<f64>, c2: &mut OpJ200<f64>) {
    let scheme = a1.scheme();
...
/// ```
pub fn c221(alpha: f64, a2: &OpJ200<f64>, b1: &OpJ100<f64>, c2: &mut OpJ200<f64>) {
    let scheme = a2.scheme();
    for rs in scheme.states_20(&occ::ALL2) {
...
/// ```
pub fn c2220(alpha: f64, a2: &OpJ200<f64>, b2: &OpJ200<f64>, c2: &mut OpJ200<f64>) {
...
/// ```
pub fn c2222(alpha: f64, a2: &OpJ200<f64>, b2: &OpJ200<f64>, c2: &mut OpJ200<f64>) {
...
pub fn monopole_elem(h2: &OpJ200<f64>, p: StateJ10, q: StateJ10) -> f64 {
    ...
    let x: f64 = jpqs
        .clone()
        .flat_map(|jpq| {
            p.combine_with_10(q, jpq)
                .map(|pq| jpq.weight(2) * h2.at(pq, pq))
        })
        .sum();
...
/// `eta`.
pub fn white_gen(denom_type: DenomType, alpha: f64, h: &MopJ012<f64>, eta: &mut MopJ012<f64>) {
    ...
    for a in scheme.states_10(&[occ::A]) {
        for i in a.costates_10(&[occ::I]) {
            let x = alpha * h.1.at(a, i) / (h.1.at(a, a) - h.1.at(i, i) - monopole(a, i));
            eta.1.add(a, i, x);
            eta.1.add(i, a, -x);
        }
    }
    for ab in scheme.states_20(&[occ::AA]) {
        ...
        for ij in ab.costates_20(&[occ::II]) {
            ...
            let x = alpha * h.2.at(ab, ij)
                / (h.1.at(a, a) + h.1.at(b, b) - h.1.at(i, i) - h.1.at(j, j) + monopole(a, b)
                    - monopole(a, i)
                    - monopole(b, i)
                    + monopole(i, j)
                    - monopole(a, j)
